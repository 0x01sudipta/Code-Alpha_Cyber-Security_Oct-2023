...
def Klyda():
    ...
    if not args.url:
        exit("(ABORT) No target URL has been specified? Use -h for Brute Alpha usage.\n")
    if not args.username and not args.usernamefile:
        exit("(ABORT) No usernames have been specified? Use -h for Brute Alpha usage.\n")
    if not args.password and not args.passwordfile:
        exit("(ABORT) No passwords have been specified? Use -h for Brute Alpha usage.\n")
    if not args.formdata:
        exit("(ABORT) No formdata has been specified? Use -h for Brute Alpha usage.\n")
    ...
    if args.usernamefile:
        for i in args.usernamefile:
            try:
                ...
                exit("(ABORT) Brute Alpha couldn't locate file, " + i)
    if args.password:
